...
public class CastleMapGenerator {
    ...
    private void initializeRooms(Floor floor) {
        for (int x = 0; x < floor.getWidth(); x++) {
            for (int y = 0; y < floor.getHeight(); y++) {
                ...
                floor.setRoom(x, y, new Room(x, y, floor.getFloorNumber(), RoomType.NORMAL, description));
            }
        }
    }
    ...
    private String generateRoomDescription(int floor, int x, int y) {
        String[] descriptions = {
            "A dusty chamber filled with ancient relics.",
            "A cold, stone room echoing with distant screams.",
            "A dimly lit hall with flickering torches.",
            "A cramped corridor lined with cracked statues."
        };
